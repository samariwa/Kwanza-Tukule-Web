...
//validate username
    if (!($fetch = mysqli_fetch_array(mysql_query("SELECT `username` FROM `users` WHERE `username`='$user'")))) {
...
//Grab login attempts from MySQL database for a corresponding username
        $result1 = mysqli_query("SELECT `loginattempt` FROM `authentication` WHERE `username`='$user'");
...
//username is registered in database, now get the hashed password
        ...
        $result = mysqli_query("SELECT `password` FROM `authentication` WHERE `username`='$user'");
...
//insert failed attempts
                ...
                mysqli_query("INSERT INTO `ipcheck` (`loggedip`, `failedattempts`) VALUES ('$iptocheck', '$loginattempts_total')");
            } else {
...
//Possible brute force attacker is targeting randomly
        ...
        if ($registered == FALSE) {
            if (!($fetch = mysqli_fetch_array(mysql_query("SELECT `loggedip` FROM `ipcheck` WHERE `loggedip`='$iptocheck'")))) {
